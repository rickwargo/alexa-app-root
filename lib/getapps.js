...
/**
 * Returns GUID for random generation of appId's and userId's
 * @returns {string}
 */
function guid() {
    function s4() {
    ...
    Object.keys(server.apps).forEach(function (name) {
        if (!appName || name === appName) {
            ...
            apps.push({
                ...
                userId: 'amzn1.ask.account.YA615MHN9RTRQM321OEBFVBZ72JXQ9LSRYW21ZZ6Z9DN7DKZ',
                sessionId: 'sessionId.' + guid(),
                requestId: 'amzn1.echo-api.request.' + guid(),
                locale: 'en-US'
            });
        }
    });
